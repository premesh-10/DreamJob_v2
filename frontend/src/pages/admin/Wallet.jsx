        ...
        <AdminLayout>
            ...
            <div className="max-w-7xl mx-auto space-y-6">
                <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                    <div>
                        <h1 className="text-3xl font-bold text-slate-900">Wallet & Withdrawals</h1>
                        <p className="text-slate-500">Platform-wide wallet balances and withdrawal approvals</p>
                    </div>
                    {/* Tab switcher */}
                    <div className="flex gap-1 bg-slate-100 rounded-xl p-1">
                        ...
                        <button onClick={() => setTab('balances')}
                            className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${tab === 'balances' ? 'bg-white shadow text-slate-900' : 'text-slate-500'}`}>
                            💰 Balances
                        </button>
                    </div>
                </div>
                ...
                {!loading && data && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                        {[
                            { label: 'Total Platform Balance', value: `$${data.totalPlatformBalance?.toFixed(2)}`, icon: '💰', color: 'from-emerald-500 to-emerald-600' },
                            { label: 'Total Volume Processed', value: `$${data.totalVolume?.toFixed(2)}`, icon: '📊', color: 'from-blue-500 to-blue-600' },
                            { label: 'Pending Withdrawals', value: pendingCount, icon: '⏳', color: pendingCount > 0 ? 'from-amber-500 to-amber-600' : 'from-slate-400 to-slate-500' },
                        ].map(c => (
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                            {loading ? <div className="p-10 text-center text-slate-400">Loading...</div> : (
                                <div className="overflow-x-auto">
                                    <table className="w-full text-sm text-left">
                                        <thead className="bg-slate-50 border-b border-slate-200">
                                            <tr>{['User', 'Email', 'Role', 'Wallet Balance'].map(h => <th key={h} className="px-5 py-4 text-slate-500 font-medium">{h}</th>)}</tr>
                                        </thead>
                                        <tbody className="divide-y divide-slate-100">
                                            {filtered.length === 0 ? (
                                                <tr><td colSpan={4} className="text-center py-10 text-slate-400">No users with wallet balance found</td></tr>
                                            ) : filtered.map(u => (
                                                <tr key={u._id} className="hover:bg-slate-50">
                                                    <td className="px-5 py-4 font-semibold text-slate-800">{u.name}</td>
                                                    <td className="px-5 py-4 text-slate-500">{u.email}</td>
                                                    <td className="px-5 py-4"><span className={`px-2.5 py-1 rounded-full text-xs font-bold ${roleColors[u.role] || 'bg-gray-100 text-gray-600'}`}>{u.role}</span></td>
                                                    <td className="px-5 py-4 font-bold text-emerald-600 text-base">${u.walletBalance?.toFixed(2)}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
