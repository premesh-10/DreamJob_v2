                    ...
                    <div key={attempt._id} className={`bg-white rounded-2xl border-2 p-5 flex flex-col sm:flex-row sm:items-center gap-4 transition hover:shadow-md ${attempt.passed ? 'border-emerald-100' : attempt.status === 'completed' ? 'border-red-100' : 'border-slate-100'}`}>
                        ...
                        {attempt.status === 'completed' && (
                            <Link
                                ...
                                className="flex-shrink-0 px-5 py-2.5 bg-gradient-to-r from-violet-600 to-indigo-600 text-white rounded-xl text-sm font-bold hover:shadow-md transition"
                            >
                                View Report →
                            </Link>
                        )}
    ...
    return (
        <Layout>
            <div className="max-w-4xl mx-auto space-y-6 py-4">
                ...
                <div className="bg-white border border-slate-200 rounded-2xl p-2 flex gap-2 shadow-sm">
                    {TABS.map(t => (
                        <button
                            ...
                            className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition ${activeTab === t.key ? 'bg-violet-600 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}
                        >
                            {t.label}
                        </button>
                    ))}
                ...
                {/* Tab Content */}
                {activeTab === 'practice' ? <PracticeReports /> : <InterviewReports />}
            </div>
        </Layout>
    );
}
